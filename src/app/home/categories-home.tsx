...
function CategoriesHome(props: { images: string[], categories: ICategoryHome[] }) {
    const [cateChoose, setCateChoose] = useState('1');
    let changeCate = (idCate: string) => {
        setCateChoose(idCate);
    }
    let dataCateChoose: ICategoryHome = props.categories.find(item => (item.id).toString() === cateChoose) || props.categories[0];
    return (
        <div>
            <div className='flex flex-col py-[25px] md:flex-row justify-center gap-3 '>
                {props.images && props.images.map((item, index) => (
                    <div key={index}>
                        <img className='sm:w-full object-cover md:w-auto' src={item} alt="img" />
                    </div>
                ))}
            </div>
            <div className='w-[80%] mx-auto'>
                <ul className='flex category-main justify-center gap-8'>
                    <li className={cateChoose === '1' ? 'li-select' : ''} onClick={() => changeCate('1')}>NAM</li>
                    <li className={cateChoose === '2' ? 'li-select' : ''} onClick={() => changeCate('2')}>NỮ</li>
                    <li className={cateChoose === '3' ? 'li-select' : ''} onClick={() => changeCate('3')}>TRẺ EM</li>
