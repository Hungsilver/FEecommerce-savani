...
export function AnonymousInfo({ isAuthenticated }: IHeaderProps) {
    ...
    const styleIcon: string = "bg-brown-50 rounded-full p-2 h-[18px] w-[18px] cursor-pointer";
...
function MenuItemAnonymousApi(props: { data: ICategory }) {
    ...
    return (
        <Menu allowHover open={isMenuOpen} handler={setIsMenuOpen} >
            <MenuHandler >
                <a href='#' className='flex items-center item-navbar-link border-none'>
                    <span>
                        {data.name}
                    </span>
                </a>
            ...
            <MenuList className='w-full flex hover:border-none' placeholder={""}>
                ...
                <div className='flex justify-between w-full'>
                    {data.categoriesChild.map((item) => (
                        <div key={item.id} className='px-8'>
                            <a href='#' className='font-bold uppercase text-black hover:text-red-500'>{item.name}</a>
                            <ul className='mt-2 flex flex-col gap-2'>
                                {item && item.subCategories.map((item) => (
...
function AnonymousMenu(props: { data: ICategory[] }) {
    ...
    return (
        <div className='md:flex justify-between w-full'>
            ...
            <div className='flex items-center item-navbar-link'>   {/*custom css */}
                <a href="#">
                    Bộ sưu tập
                </a>
